<script>
  // Form for adding a new item, either via scan or manual
  ...
  // For manual creation of new item
  export let item = {
    name: "",
    category: "",
    quantity: 0,
  ...
  function save() {
    ...
    if (imageData && imageData.startsWith("data:")) {
      ...
      try {
        ...
        console.warn(
          "Failed to save image to localStorage (might be too large):",
          e
        );
        // Let user know image COULD be too large
        alert(
          "Warning: Image may be too large to save. Consider using a smaller image."
        );
      }
    }

    // Update items store with new item
    items.update((prev) => {
      const newItem = {
        ...item,
        // Give new item the next id in the pattern
        id: getNewId(),
  ...
  function handleFileSelect(event) {
    ...
    if (file && file.type.startsWith("image/")) {
      // Check file size (limit to 1MB to not clog local storage)
...
<section class="form-container">
  <div class="form">
    ...
    <label
      >Quantity<input type="number" bind:value={item.quantity} min="0" /></label
    >
